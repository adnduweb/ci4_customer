...
use Adnduweb\Ci4_customer\Exceptions\AuthException;
 ...
 *
 * @package Adnduweb\Ci4_customer\Authentication\Passwords\Validators
 */
class NothingPersonalValidator extends BaseValidator implements ValidatorInterface
{
     ...
     * comes from Adnduweb\Ci4_customer\Entities\User properties username and email.
     *
     ...
     * the `$personalFields` property in Adnduweb\Ci4_customer\Config\Auth, e.g.
     *
    ...
    protected function isNotPersonal($password, $user)
    {
        ...
        $this->error = lang('Authcustomer.errorPasswordPersonal');
        $this->suggestion = lang('Authcustomer.suggestPasswordPersonal');
        return false;
    }

    /**
     * notSimilar() uses $password and $userName to calculate a similarity value.
     * Similarity values equal to, or greater than Adnduweb\Ci4_customer\Config::maxSimilarity
    ...
    protected function isNotSimilar($password, $user)
    {
        ...
        if (!empty($maxSimilarity)) {
            ...
            if ($similarity >= $maxSimilarity) {
                $this->error = lang('Authcustomer.errorPasswordTooSimilar');
                $this->suggestion = lang('Authcustomer.suggestPasswordTooSimilar');
                return false;
            }
        }
        return true;
