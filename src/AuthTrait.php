...
namespace Adnduweb\Ci4_customer\Auth;
...
trait AuthTrait
{
    ...
    public function restrict(string $uri = null, bool $returnOnly = false)
    {
        ...
        if (method_exists($this, 'setMessage')) {
            session()->setFlashdata('error', lang('Authcustomer.notLoggedIn'));
        }
    ...
    public function restrictToGroups($groups, $uri = null)
    {
        ...
        if (method_exists($this, 'setMessage')) {
            session()->setFlashdata('error', lang('Authcustomer.notEnoughPrivilege'));
        }
