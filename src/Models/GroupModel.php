...
class GroupModel extends Model
{
    ...
    {
        if (! $found = cache("{$groupId}_authf_customer"))
        {
            $found = $this->builder()
                ->select('authf_groups_users.*, authf_customer.*')
                ->join('authf_groups_users', 'authf_groups_users.group_id = authf_groups.id', 'left')
                ->join('authf_customer', 'authf_groups_users.user_id = authf_customer.id', 'left')
                ->where('authf_groups.id', $groupId)
                ->get()->getResultArray();

            cache()->save("{$groupId}_authf_customer", $found, 300);
        }
