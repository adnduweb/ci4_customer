...
class LoginModel extends Model
{
    ...
    protected $allowedFields = [
        'ip_address', 'email', 'customer_id', 'date', 'success'
    ];
    ...
    public function rememberUser(int $userID, string $selector, string $validator, string $expires)
    {
        ...
        return $this->db->table('authf_tokens')->insert([
            'customer_id' => $userID,
            'selector' => $selector,
    ...
    {
        return $this->db->table('authf_tokens')
    ...
    {
        return $this->builder('authf_tokens')->where(['customer_id' => $id])->delete();
    }

    /**
     * Purges the 'authf_tokens' table of any records that are past
